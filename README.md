...
В этом режиме создаётся `site-model.json` с найденными заголовками, ссылками, формами, полями, кнопками, изображениями, статусом страницы и, если доступен Playwright, дополнительными данными браузера. Затем создаётся synthetic `unified-model.json`, чтобы обычная генерация документов могла подготовить exploratory/smoke/UI тест-кейсы. В ответе агент явно предупреждает: тест-кейсы созданы по фактическому поведению сайта, а не по требованиям.
