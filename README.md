...
| `test-cases-quality-report.json` | Локальная оценка качества тест-кейсов перед экспортом |
| `checklist-quality-report.json` | Локальная оценка качества чек-листа перед экспортом |
...
Агент получает строгий QA-standard prompt и старается сразу генерировать профессиональные, атомарные и исполнимые тест-кейсы: одна проверка на кейс, конкретные шаги, наблюдаемый expected result, source_refs и тестовые данные там, где есть ввод или негативная проверка.

После генерации запускается локальная проверка качества без LLM и внешних API. Quality Gate проверяет атомарность, конкретность названий и шагов, expected result, source_refs, тестовые данные, vague phrases вроде «работает корректно», возможные выдуманные требования и пригодность к будущей автоматизации.

Слабые кейсы и пункты чек-листа не удаляются автоматически: они получают статус `needs_review` или `weak`, score и список issues. Отчёты сохраняются как `test-cases-quality-report.json` / `checklist-quality-report.json` с учётом focus (`test-cases-quality-report-smoke.json` и т.п.).

Quality status также попадает в `note`, поэтому виден в колонке «Примечание» в `test-cases.csv` / `test-cases.xlsx` и checklist export.
