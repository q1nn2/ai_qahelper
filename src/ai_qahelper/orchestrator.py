...
def generate_docs(
    ...
    analysis_json = session_dir / "test-analysis.json"
    if run_analysis:
        try:
            analysis = _retry(
                2,
    ...
    export_test_cases_local(session_dir, test_cases, cfg.test_cases_export)
...
def _parse_sheet(url: str) -> tuple[str, str]:
    ...
    gid = _worksheet_gid_from_url(parsed)
    return spreadsheet_id, gid


def _worksheet_gid_from_url(parsed: ParseResult) -> str:
    """gid из ?gid= (query) или из #gid= (fragment); иначе лист по умолчанию."""
